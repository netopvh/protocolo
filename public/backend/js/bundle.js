...
$(function () {
    ...
    if (departamento.length) {
        departamento.DataTable({
            ...
            columns: [{ data: 'id', width: '30px' }, { data: 'descricao' }, { data: 'action', orderable: false, searchable: false, width: '115px' }]
        });
    }
    ...
    //DASHBOARD TABLE
    var dashboardTbl = $('#tbl_dashboard');
    if (dashboardTbl.length) {
        var oTableDash = dashboardTbl.DataTable({
            dom: "<'row'<'col-xs-12'<'col-xs-12'>>r>" + "<'row'<'col-xs-12't>>" + "<'row'<'col-xs-12'<'col-xs-6'i><'col-xs-6'p>>>",
            serverSide: true,
            processing: true,
            responsive: true,
            language: dt_trans,
            ajax: '/documentos',
            columns: [{ data: 'numero', name: 'documentos.numero', width: '80px' }, { data: 'prioridade', name: 'documentos.prioridade' }, { data: 'tipo', name: 'tipo_documentos.descricao' }, { data: 'origem' }, { data: 'data_doc', name: 'documentos.data_doc', width: '180px' }, { data: 'action', orderable: false, searchable: false, width: '70px' }],
        ...
        var tbDashboard = $('table[data-form="tbDashboard"]');
    ...
    if (documento.length) {
        var oTable = documento.DataTable({
            ...
            columns: [{ data: 'numero', name: 'documentos.numero', width: '80px' }, { data: 'assunto', name: 'documentos.assunto' }, { data: 'tipo', name: 'tipo_documentos.descricao' }, { data: 'origem' }, { data: 'data_doc', name: 'documentos.data_doc', width: '180px' }, { data: 'action', orderable: false, searchable: false, width: '150px' }]
        });
    ...
    if (documentoPend.length) {
        var oTableP = documentoPend.DataTable({
            ...
            columns: [{ data: 'numero', name: 'documentos.numero', width: '80px' }, { data: 'assunto', name: 'documentos.assunto' }, { data: 'tipo', name: 'tipo_documentos.descricao' }, { data: 'origem' }, { data: 'data_doc', name: 'documentos.data_doc', width: '180px' }, { data: 'action', orderable: false, searchable: false, width: '130px' }]
        });
        ...
        var tbPendente = $('table[data-form="tbPendente"]');
        ...
        tbPendente.on('click', '.receber', function (e) {
            e.preventDefault();
            _title.html('');
            _title.html('Recebimento de documentos');
            $('#title-modal').show();
            $('.despacho').hide();
            var data = {
                _token: $('meta[name="csrf-token"]').attr('content'),
        ...
        tbPendente.on('click', '.devolver', function (e) {
            ...
            e.preventDefault();
            _title.html('');
            _title.html('Devolução de documentos');
            $('#title-modal').hide();
            $('.despacho').show();
    ...
    var documentoArquiv = $('#tbl_doc_arquivado');
    if (documentoArquiv.length) {
        var oTableA = documentoArquiv.DataTable({
            dom: "<'row'<'col-xs-12'<'col-xs-12'>>r>" + "<'row'<'col-xs-12't>>" + "<'row'<'col-xs-12'<'col-xs-6'i><'col-xs-6'p>>>",
            serverSide: true,
            processing: true,
            responsive: true,
    ...
    //DOCUMENTOS ENVIADOS
    var documentEnviado = $('#tbl_doc_enviado');
    if (documentEnviado.length) {
        var oTableEnv = documentEnviado.DataTable({
            dom: "<'row'<'col-xs-12'<'col-xs-12'>>r>" + "<'row'<'col-xs-12't>>" + "<'row'<'col-xs-12'<'col-xs-6'i><'col-xs-6'p>>>",
            serverSide: true,
            processing: true,
            responsive: true,
            ...
            columns: [{ data: 'numero', name: 'documentos.numero', width: '60px' }, { data: 'assunto', name: 'documentos.assunto' }, { data: 'tipo', name: 'tipo_documentos.descricao' }, { data: 'destino' }, { data: 'data_doc', name: 'documentos.data_doc', width: '180px' }, { data: 'action', orderable: false, searchable: false, width: '130px' }]
        });
    ...
    var getDespacho = $('.despacho');
    getDespacho.on("click", function (e) {
        var tramitacaoId = $(this).data('id');
        var body = $('.modal-body');
        $.ajax({
            ...
            success: function success(response) {
                if (response.status === 'error') {
                    ...
                    $.each(response.tramitacoes, function (key, value) {
                        $('ul.list-feed').append('<li class="media">' + '<div class="media-body">' + '<div class="text-muted">' + value.data_tram + '</div>' + '<span class="text-bold">' + value.usuario + ' (' + value.departamento + ')</span>' + value.acao + '</div>' + '</li>');
                    });
                }
            }
        });
...
__webpack_require__("./resources/assets/js/core/app.js");
__webpack_require__("./resources/assets/js/pages/layout_fixed_custom.js");
module.exports = __webpack_require__("./resources/assets/js/plugins/ui/ripple.min.js");
